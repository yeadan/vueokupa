    ...
    <div class="col-lg-6 col-md-12">
      ...
      <table>
        ...
        <tr>
          ...
          <td><textarea v-model="resultado.description" placeholder="Añade una descripción..." name="prop_descripcion" rows="5" cols="30" minlength="3" maxlength="200"></textarea></td>
        </tr>
      </table>
      <div v-if="(resultado.owner_id > 0) 
                        && (resultado.user_id > 0)
                        && (resultado.okupa_id > 0)
                        && (Object.keys(resultado.description).length > 2)
                        && (Object.keys(resultado.type).length > 0) ">
      <span>Añadir dirección<button class="btn btn-success"  onclick="document.getElementById('select_com').removeAttribute('disabled');">Añadir</button></span>
      </div>
      <div v-else>
      <span>Añadir dirección<button  disabled class="btn btn-success" >Añadir</button> </span>
  ...
  <div id="div_address" class="col-lg-6 col-md-12" >  
    ...
    <table>
      <tr>
        ...
        <td><select disabled id="select_com"  class="" @change="get_provincias()" v-model="selectThis.com">
          <option value="0" disabled selected>Escoge una...</option>
          <option v-for="(comunidad, index) in comunidades" :key="index" :value="comunidad.CCOM">{{comunidad.COM}}</option>
        </select></td>
      </tr>
      <tr>
        <td><label class="" for="select_pro">Provincia </label></td>
        <td><select disabled id="select_pro"  class="" @change="get_municipios()" v-model="selectThis.pro">
          <option value="0" disabled selected>Escoge una...</option>
          <option v-for="(provincia, index) in provincias" :key="index" :value="provincia.CPRO">{{provincia.PRO}}</option>
        </select></td>
      </tr>
      <tr>
        <td><label class="" for="select_mun">Municipio </label></td>
        <td><select disabled id="select_mun"  class="" @change="get_poblaciones()" v-model="selectThis.mun" >
          <option value="0" disabled selected>Escoge uno...</option>
          <option v-for="(municipio, index) in municipios" :key="index" :value="municipio.CMUM">{{municipio.DMUN50}}</option>
        </select></td>
      </tr>
      <tr>
        <td><label class="" for="select_pob">Población </label></td>
        <td><select disabled id="select_pob"  class="" @change="get_nucleos()" v-model="selectThis.pob">
          <option value="0" disabled selected>Escoge una...</option>
          <option v-for="(poblacion, index) in poblaciones" :key="index" :value="poblacion.NENTSI50">{{poblacion.NENTSI50}}</option>
        </select></td>
      </tr>
      <tr>
        <td><label class="" for="select_nuc">Núcleo </label></td>
        <td><select disabled id="select_nuc"  class="" @change="get_CPs()" v-model="selectThis.nuc">
          <option value="0" disabled selected>Escoge una...</option>
          <option v-for="(nucleo, index) in nucleos" :key="index" :value="nucleo.CUN">{{nucleo.NNUCLE50}}</option>
        </select></td>
      </tr>
      <tr>
        <td><label class="" for="select_cps">Código Postal </label></td>
        <td><select disabled id="select_cps"  class="" @change="get_calles()" v-model="selectThis.CPs">
          <option value="0" disabled selected>Escoge uno...</option>
          <option v-for="(CP, index) in codigosPostal" :key="index" :value="CP.CPOS">{{CP.CPOS}}</option>
        </select></td>
      </tr>
      <tr>
        <td><label class="" for="select_cal">Dirección </label></td>
        <td><select disabled id="select_cal"  class="" @change="get_end()" v-model="selectThis.cal">
          <option value="0" disabled selected>Escoge una...</option>
...
export default {
  data () {
    return {
      ...
      resultado:{owner_id:0,okupa_id:0,user_id:parseInt(this.$store.getters.getUserID),type:0,description:'',created:0,comunidad:'',provincia:'',municipio:'', poblacion:'', nucleo:'', codigo_postal:'',calle:'',numero:0,piso:0,puerta:'',okupa:{},owner:{}},
  ...
  methods: {
    get_comunidades() {
      ...
      const self = this
      const url = 'https://apiv1.geoapi.es/comunidades?type=JSON&key='+this.key+'&sandbox=0'
    ...
    get_provincias() {
      ...
      this.resultado.comunidad = document.getElementById('select_com').options[document.getElementById('select_com').selectedIndex].text
      const self = this
      const url = 'https://apiv1.geoapi.es/provincias?CCOM='+ID+'&type=JSON&key='+this.key+'&sandbox=0'
      this.instance.get(url)
      .then(response => {
    ...
    get_municipios() {
      ...
      this.resultado.provincia = document.getElementById('select_pro').options[document.getElementById('select_pro').selectedIndex].text
      const self = this
      const url = 'https://apiv1.geoapi.es/municipios?CPRO='+ID+'&type=JSON&key='+this.key+'&sandbox=0'
      this.instance.get(url)
      .then(response => {
    ...
    get_poblaciones() {
      ...
      this.resultado.municipio = document.getElementById('select_mun').options[document.getElementById('select_mun').selectedIndex].text
      const self = this
      const url = 'https://apiv1.geoapi.es/poblaciones?CPRO='+IDPRO+'&CMUM='+ID+'&type=JSON&key='+this.key+'&sandbox=0'
      this.instance.get(url)
      .then(response => {
    ...
    get_nucleos() {
      ...
      const self = this
      const url = 'https://apiv1.geoapi.es/nucleos?CPRO='+IDPRO+'&CMUM='+ID+'&NENTSI50='+IDPOB+'&type=JSON&key='+this.key+'&sandbox=0'
      this.instance.get(url)
      .then(response => {
    ...
    get_CPs() {
      ...
      let ID = this.selectThis.mun
      this.resultado.nucleo = document.getElementById('select_nuc').options[document.getElementById('select_nuc').selectedIndex].text
      const self = this
      const url = 'https://apiv1.geoapi.es/cps?CPRO='+IDPRO+'&CMUM='+ID+'&CUN='+IDNUC+'&type=JSON&key='+this.key+'&sandbox=0'
      this.instance.get(url)
      .then(response => {
    ...
    get_calles() {
      ...
      const self = this
      const url = 'https://apiv1.geoapi.es/calles?CPRO='+IDPRO+'&CMUM='+ID+'&CUN='+IDNUC+'&CPOS='+IDCP+'&type=JSON&key='+this.key+'&sandbox=0'
     this.instance.get(url)
      .then(response => {
    ...
    get_end() {
      ...
      this.resultado.calle = document.getElementById('select_cal').options[document.getElementById('select_cal').selectedIndex].text
    },
    get_number(value) {
    ...
    comprobate() {
      for(var propt in this.resultado){
        if (this.resultado[propt].length <1 && propt != 'puerta') 
           {
              document.getElementById('btn_add').setAttribute('class',"btn btn-warning")
              setTimeout(() => {
                document.getElementById('btn_add').setAttribute('class',"btn btn-success")
                  }, 500)
              return
           }
      }
